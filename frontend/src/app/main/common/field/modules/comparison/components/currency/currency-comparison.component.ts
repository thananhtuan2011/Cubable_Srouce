...
const comparisonRange: ReadonlySet<ComparisonOperator> = new Set([
	ComparisonOperator.IS_BETWEEN,
	ComparisonOperator.IS_NOT_BETWEEN,
]);

const comparisonEmpty: ReadonlySet<ComparisonOperator> = new Set([
	ComparisonOperator.ANY,
	ComparisonOperator.IS_EMPTY,
	ComparisonOperator.IS_NOT_EMPTY,
]);

export type CurrencyData = AdvanceData & {
	// static
	number?: number;
...
@Component({
	selector: 'currency-comparison',
	templateUrl: './currency-comparison.pug',
	host: { class: 'currency-comparison' },
	styleUrls: [ '../../styles/comparison-base.scss' ],
	changeDetection	: ChangeDetectionStrategy.OnPush,
})
export class CurrencyComparisonComponent
	extends ComparisonBase<CurrencyData> {
	...
	protected readonly comparisonRange:
	ReadonlySet<ComparisonOperator>
		= comparisonRange;
	protected readonly fb: FormBuilder
		= inject( FormBuilder );

	public rangeForm: FormGroup = this.fb.group({
		start: undefined,
		end: undefined,
	});
	...
	protected override onOperatorChange() {
		...
		this.data = this.operatorHasComparisonType.has( this.operator )
			? {
				compareType: CurrencyComparisonComponent.default.compareType,
			}
